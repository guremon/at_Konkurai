                    ...
                    for (var i = 0; i < settings.columns.editable.length; i++) {
                        ...
                        $td.each(function() {
                            ...
                            if (typeof settings.columns.editable[i][2] !== 'undefined') {
                                ...
                                var input = '<input class="tabledit-input ' + settings.inputClass + '" type="text" name="' + settings.columns.editable[i][1] + '" value="' + text + '" style="display: none;" disabled>';
                            }
                ...
                toolbar: function() {
                    if (settings.editButton || settings.deleteButton) {
                        ...
                        if (settings.deleteButton) {
                            ...
                            confirmButton = '<button type="submit" name="delete" class="submitForDelete tabledit-confirm-button ' + settings.buttons.confirm.class + '" style="display: none; float: none;">' + settings.buttons.confirm.html + '</button>';
                        }

                        // Create save button.
                        if (settings.editButton && settings.saveButton) {
                            saveButton = '<button type="submit" name="update" class="submitForUpdate tabledit-save-button ' + settings.buttons.save.class + '" style="display: none; float: none;">' + settings.buttons.save.html + '</button>';
                        }
...
//                        }
                        ...
                        var toolbar = '<div class="tabledit-toolbar ' + settings.toolbarClass + '" style="text-align: left;">\n\
                                           <div class="' + settings.groupClass + '" style="float: none;">' + editButton + deleteButton + saveButton + confirmButton + restoreButton +'</div>\n\
                                       </div></div>';
        ...
        var Mode = {
            view: function(td) {
                ...
                if (settings.editButton) {
                    ...
                    $(td).parent('tr').removeClass(settings.successClass);
                    $tr.find('button.tabledit-edit-button').removeClass('active').blur();
                }
                $(td).parent('tr').removeClass(settings.dangerClass);
            },
            edit: function(td) {
                Delete.reset(td);
                // Get table row.
                ...
                // Add "success(select)" class in td element.
                $(td).parent('tr').addClass(settings.successClass);
                // Update toolbar buttons.
                if (settings.editButton) {
        ...
        var Edit = {
            reset: function(td) {
                $(td).each(function() {
                    ...
                    // Release 'readonly' attribute from input.
                    $input.prop('readonly', false);
                    // Release 'disabled' attribute from selector.
                    $table.find('.category').prop('disabled', false);
        ...
        var Delete = {
            reset: function(td) {
                ...
                // Remove dangerClass in delete button.
                $(td).parent('tr').removeClass(settings.dangerClass);
            },
            submit: function(td) {
                Delete.reset(td);
...
//                });
                ...
                // Hide edit button.
                $(td).find('.tabledit-save-button').hide();
                // Show confirm button.
                $(td).find('.tabledit-delete-button').show();
                // Show confirm button.
                $(td).find('.tabledit-confirm-button').show();
                // Add dangerClass in select row
                $(td).parent('tr').addClass(settings.dangerClass);
            },
        };
        ...
        if (settings.deleteButton) {
            ...
            $table.on('click', 'button.tabledit-delete-button', function(event) {
                if (event.handled !== true) {
                    ...
                    // Change the button text from 'Cancel!' To 'Delete!'.
                    $(this).text("Delete!");
                    ...
                    if (!activated) {
                        ...
                        // Change the button text from 'Delete!' To 'Cancel!'.
                        $button.text("Cancel!");
                        ...
                        Delete.confirm($td);
                        $table.find('.tabledit-input:visible').prop('readonly', true);
                        $table.find('.category').prop('disabled', true);
        ...
        if (settings.editButton) {
            ...
            $table.on('click', 'button.tabledit-edit-button', function(event) {
                if (event.handled !== true) {
                    ...
                    // Change the button text from 'Cancel!' To 'Edit!'.
                    $button.text("Edit!");
                    ...
                    if (!activated) {
                        ...
                        // Change the button text from 'Edit!' To 'Cancel!'.
                        $button.text("Cancel!");
                    }
            ...
            $table.on('click', 'button.tabledit-save-button', function(event) {
                if (event.handled !== true) {
                    ...
                    $(this).parents('tr').find('button.tabledit-delete-button').show();
                    event.handled = true;
                }
            });
        } else {
